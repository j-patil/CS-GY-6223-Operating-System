...
int keys[NUM_KEYS];
pthread_mutex_t locks[NUM_BUCKETS]; // Declaring a lock for every bucket in this statement
...
void insert(int key, int val) {
    ...
    pthread_mutex_lock(&locks[i]); //Acquiring a lock at that particular bucket
...
int main(int argc, char **argv) {
    ...
    for(i=0;i<NUM_BUCKETS;i++){
        pthread_mutex_init(&locks[i], NULL);} //Initialize the lock here.
